...
class ShopPartnerController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'topMenuRight' => [],
            'topMenuLeft' => [],
            'urlDeleteItem' => route('admin_partner.delete'),
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[] = [
                'image' => sc_image_render($row->getThumb(), '', '50px', 'Partner'),
                'url' => $row['url'],
                'sort' => $row['sort'],
                'status' => $row['status'] ? '<span class="label label-success">ON</span>' : '<span class="label label-danger">OFF</span>',
                'target' => $row['target'],
                'action' => '
                    <a href="' . route('admin_partner.edit', ['id' => $row['id']]) . '"><span title="' . trans('banner.admin.edit') . '" type="button" class="btn btn-flat btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
...
//menuRight
        $data['menuRight'][] = '<a href="' . route('admin_partner.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
...
//menuSearch        
        ...
        $data['urlSort'] = route('admin_partner.index');
 ...
 */
    public function create()
    {
        $data = [
            ...
            'url_action' => route('admin_partner.create'),
        ];
        return view('admin.screen.partner')
            ->with($data);
    }
 ...
 */
    public function postCreate()
    {
        ...
        ShopPartner::create($dataInsert);

        return redirect()->route('admin_partner.index')->with('success', trans('banner.admin.create_success'));
 ...
 */
    public function edit($id)
    {
        ...
        $data = [
            ...
            'dataType' => $this->dataType,
            'banner' => $banner,
            'url_action' => route('admin_partner.edit', ['id' => $banner['id']]),
        ];
        return view('admin.screen.partner')
            ->with($data);
    }
...
//Edit
        ...
        $obj = ShopPartner::find($id);
        $obj->update($dataUpdate);

//
        return redirect()->route('admin_partner.index')->with('success', trans('banner.admin.edit_success'));
